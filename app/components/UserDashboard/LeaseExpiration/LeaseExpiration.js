...
const LeaseExpiration = () => {
  ...
  useEffect(() => {
    const fetchExpiringLeases = async () => {
      ...
      try {
        const response = await fetch('https://djang-backend-final-project.onrender.com/api/v1/leases/expiring-leases/', {
          headers: {
        ...
        if (response.status === 401) {
          ...
          if (accessToken) {
            ...
            const retryResponse = await fetch('https://djang-backend-final-project.onrender.com/api/v1/leases/expiring-leases/', {
              headers: {
                'Authorization': `Bearer ${accessToken}`,
              }
            });

...
const usePropertyResource = () => {
    ...
    const deleteProperty = async (propertyId) => {
        try {
            await axios.delete(`http://localhost:8000/api/v1/properties/${propertyId}`);
    ...
    const addProperty = async (propertyData) => {
        try {
            ...
            // Make POST request
            const response = await axios.post('http://localhost:8000/api/v1/properties/create/', propertyData, {
                headers: { 'Content-Type': 'application/json' },
            });
